...
import type { UpdateEventInputContract } from "@/apps/api/src/modules/events/contracts/update-event.input"
...
const validEventStatuses = new Set<EventDashboardStatus>([
  "scheduled",
  "completed",
  "cancelled",
])

export class InvalidUpdateEventInputError extends Error {
  constructor(message: string) {
    super(message)
    this.name = "InvalidUpdateEventInputError"
  }
}
...
export function parseUpdateEventInput(
  body: Record<string, unknown>
): UpdateEventInputContract {
  ...
  if (!name || !date || !time || !location || !Number.isFinite(capacity) || capacity <= 0) {
    throw new InvalidUpdateEventInputError(
      "Informe nome, data, horário, local e capacidade válida."
    )
  }

  if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) {
    throw new InvalidUpdateEventInputError("Informe uma data válida para o evento.")
  }

  if (!/^\d{2}:\d{2}$/.test(time)) {
    throw new InvalidUpdateEventInputError("Informe um horário válido para o evento.")
  }

  if (!validEventTypes.has(type as EventDashboardType)) {
    throw new InvalidUpdateEventInputError("Tipo de evento inválido.")
  }

  if (!validEventStatuses.has(status as EventDashboardStatus)) {
    throw new InvalidUpdateEventInputError("Status de evento inválido.")
  }
  ...
  if (hasRegistrationFee) {
    if (
      ...
      throw new InvalidUpdateEventInputError("Informe um valor de taxa maior que zero.")
    }
  ...
  return {
    ...
    status: status as EventDashboardStatus,
  }
}
